...
class MedicalClassifier:
    ...
    def _load_custom_models(self):
        ...
        # Chest X-Ray Model (from major_project-main)
        try:
            print("Loading Chest X-Ray Model (major_project-main)...")
            chest_model_path = "major_project-main/models/chest_xray.h5"
            if os.path.exists(chest_model_path):
                try:
                    ...
                    print(f"  [OK] Chest X-Ray model loaded from {chest_model_path}")
                except Exception as e:
                    print(f"  [FAILED] Failed to load Chest X-Ray model: {e}")
                    self.models['chest_xray_custom'] = None
            else:
                print("  [INFO] Chest X-Ray model not found at expected path.")
                self.models['chest_xray_custom'] = None
        except Exception as e:
            print(f"  [FAILED] Failed to load Chest X-Ray model: {e}")
            self.models['chest_xray_custom'] = None
    ...
    def classify_chest_xray(
        ...
        results = {
            ...
            "classifications": {},
            "report": None,
            "model_used": "MedSigLIP (Primary) + CheXpert + CXR Foundation + MedGemma"
        }
        
        image = self.preprocess_image(image_path)
        
        # 1. MedGemma (Primary - Highest Accuracy)
        if self.models.get('medgemma') is not None or self.medgemma_pipeline is not None:
