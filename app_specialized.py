...
def display_results(results, show_details):
    ...
    def _render_top5_from_dict(preds: dict, title: str):
        if not preds:
            st.write("No predictions available.")
            return
        sorted_preds = sorted(preds.items(), key=lambda kv: kv[1], reverse=True)[:5]
        st.markdown(f"**{title}**")
        for k, v in sorted_preds:
            st.write(f"- **{k}**: {v:.1%}")

    def _get_chexpert_top(chexpert: dict):
        preds = chexpert.get("top_predictions") or chexpert.get("predictions") or {}
        if not preds:
            return ("N/A", 0.0, {})
        top_k, top_v = max(preds.items(), key=lambda kv: kv[1])
        return (top_k, float(top_v), preds)
    ...
    # Chest X-ray: Always show TWO separate model answers (as requested)
    if results.get("input_type") == "chest_xray":
        st.markdown("---")
        st.subheader("Two Model Answers (for comparison)")

        c1, c2 = st.columns(2)

        # 1) Custom finetuned chest model
        with c1:
            st.markdown("#### 1) Custom ChestXray (Finetuned)")
            custom = classifications.get("custom_chest") or {}
            if "error" in custom:
                st.error(custom["error"])
            else:
                top = custom.get("top_prediction", "N/A")
                conf = float(custom.get("top_confidence", 0.0))
                st.metric("Primary Finding", top)
                st.metric("Confidence", f"{conf:.1%}")
                # Also show 3rd option from custom model output (as requested)
                preds = custom.get("predictions") or {}
                if preds and len(preds) >= 3:
                    sorted_preds = sorted(preds.items(), key=lambda kv: kv[1], reverse=True)
                    third_label, third_score = sorted_preds[2]
                    st.metric("3rd Option (from your model)", third_label, f"{third_score:.1%}")
                elif preds:
                    st.info("Not enough classes to show a 3rd option.")
                if show_details:
                    _render_top5_from_dict(preds, "Top-5 (Finetuned model)")

        # 2) CheXpert
        with c2:
            st.markdown("#### 2) CheXpert DenseNet")
            chexpert = classifications.get("chexpert") or {}
            if "error" in chexpert:
                st.error(chexpert["error"])
            else:
                top, conf, preds = _get_chexpert_top(chexpert)
                st.metric("Primary Finding", top)
                st.metric("Confidence", f"{conf:.1%}")
                if show_details:
                    _render_top5_from_dict(preds, "Top-5 (CheXpert)")
