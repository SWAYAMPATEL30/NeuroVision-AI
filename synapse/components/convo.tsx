...
export function Conversation() {
  ...
  const startConversation = useCallback(async () => {
    try {
      ...
      const keys = error ? Object.getOwnPropertyNames(error).join(', ') : 'none';
      let details = '';
      try { details = JSON.stringify(error, Object.getOwnPropertyNames(error)); } catch (e) {}
      
      console.error('Failed to start conversation details:', { errorStr, keys, details, errorType: typeof error });
      alert(`Failed to start conversation. Type: ${typeof error}. String: ${errorStr}. Keys: ${keys}. Details: ${details}`);
    }
  }, [conversation]);
