...
import { CalendarDays, Clock, CheckCircle2, User, Phone, FileText, ChevronRight, ChevronLeft, Stethoscope, Video, MapPin, Loader2 } from "lucide-react";
  ...
  const handleConfirm = async () => {
    ...
    try {
      const booking = await bookAppointment(
        selectedDoctor,
        consultType === "video" ? "online" : "offline",
        { id: `${selectedDate}-${selectedTime}`, time: selectedTime, date: selectedDate, available: true },
        [], // attachedFiles
        "", // aiReport
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground">{doc.specialization} • {doc.city}</p>
                            </div>
              ...
              {step === 3 && (
                <Card>
                  ...
                  <CardContent className="grid grid-cols-2 gap-4">
                    {[
                      { id: "video", label: "Video Consult", desc: "Meet online via secure video call", icon: Video },
                      { id: "in-person", label: "In-Person", desc: "Visit the clinic at scheduled time", icon: MapPin },
                    ].map(opt => (
                      <button
                        key={opt.id}
                        onClick={() => setConsultType(opt.id as "video" | "in-person")}
                      ...
                      >
                        <opt.icon className={`w-8 h-8 mx-auto mb-3 ${consultType === opt.id ? "text-blue-400" : "text-slate-500"}`} />
                        <p className="font-bold text-lg">{opt.label}</p>
                        <p className={`text-sm mt-1 ${consultType === opt.id ? "text-blue-200" : "text-slate-500"}`}>{opt.desc}</p>
                        {consultType === opt.id && <CheckCircle2 className="w-5 h-5 text-blue-500 mx-auto mt-3" />}
                      </button>
                    ))}
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="bg-muted/30 rounded-xl p-4 text-sm space-y-1 border">
                      ...
                      <p><span className="text-muted-foreground">Doctor:</span> {selectedDoctor?.name} ({selectedDoctor?.specialization})</p>
